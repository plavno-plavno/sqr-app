  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i <= wavePoints; i++) {
        ...
        // волна по радиусу
        const wave =
          Math.sin((angle * barsCount) / 8 + micPhase * 2) *
      ...
      for (let wave = 1; wave <= 3; wave++) {
        ...
        for (let i = 0; i <= wavePoints; i++) {
          ...
          const fadeR =
            waveRadius +
            fadeWave +
            direction * 30 * wave * Math.sin(micPhase * 3);
      ...
      for (let i = 0; i <= wavePoints; i++) {
        ...
        const direction = lastMicLevel.current > 0.1 ? -1 : 1;
        const inwardR =
          waveRadius + inwardWave + direction * 40 * Math.sin(micPhase * 3);
