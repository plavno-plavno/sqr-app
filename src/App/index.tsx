...
import { ServerResponse, AudioResponse } from '../types/requests';
...
export const App = () => {
  ...
  const handleLanguageChange = async (newLanguage: string) => {
    ...
    if (wsUrl) {
      ...
      await wsConnectionRef.current.initSocket(wsUrl, () => {}, handleTranscription);
    }
  };

  const handleTranscription = (response: ServerResponse) => {
    if ('segments' in response) {
      if (typeof response.segments === 'object' && !Array.isArray(response.segments)) {
